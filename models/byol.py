...
import torchvision.transforms as tnf
from kornia import augmentation as augs
from util.gather_layers import GatherLayer
...
class RandomApply(pl.LightningModule):
...
class EMA():
    ...
    def update_average(self, old, new):
        if old is None:
            return new
        old = old.to(new.device, copy=False)
...
class MLP(pl.LightningModule):
    ...
    def forward(self, x):
        return self.net(x.to(self.device, copy=False))


class ConvMLP(pl.LightningModule):
    def __init__(self, chan, chan_out = 256, inner_dim = 2048):
        super().__init__()
        self.net = nn.Sequential(
...
class PPM(pl.LightningModule):
    ...
    def forward(self, x):
        ...
        transform_out = self.transform_net(x.to(self.device, copy=False))
        out = einsum('b x y h w, b c h w -> b c x y', similarity.to(self.device, copy=False), transform_out.to(self.device, copy=False))
        return out
...
class PixelCL(pl.LightningModule):
    def __init__(
        self,
        net,
        image_size,
        ...
        self.ourAug = tnf.RandomErasing(p=1, scale=(0.0002, 0.01), ratio=(1, 1), value='random', inplace=False)
        ...
        self._transform = transforms.Compose([transforms.Resize(size=(192, 320)),
                                              transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])])

        self.projection_size = projection_size,
        self.projection_hidden_size = projection_hidden_size,
        self.layer_pixel_ = hidden_layer_pixel,
        self.hidden_layer_instance = hidden_layer_instance


    def finetuning(self, last_model, model):
    ...
    def _get_target_encoder(self):
        target_encoder = NetWrapper(
            net=self.online_encoder.net,
            projection_size=self.online_encoder.projection_size,
            projection_hidden_size=self.online_encoder.projection_hidden_size,
            layer_pixel=self.online_encoder.layer_pixel,
            layer_instance=self.online_encoder.layer_instance
        )
        target_encoder = self.finetuning(self.online_encoder, target_encoder)
    ...
    def forward(self, x, return_positive_pairs=False):
        ...
        shape, device, prob_flip = x.shape, self.device, self.prob_rand_hflip
        ...
        image_one_cutout = flip_image_one_fn(image_one_cutout).float()
        image_two_cutout = flip_image_two_fn(image_two_cutout).float()
        ...
        coordinates = torch.meshgrid(
            torch.arange(image_h, device = self.device),
            torch.arange(image_w, device = self.device)
        )
        ...
        with torch.no_grad():
            ...
            target_proj_pixel_two, target_proj_instance_two = target_encoder(image_two_cutout.to(image_one_cutout.device, copy=False))
        ...
        pred_instance_one = torch.cat(GatherLayer.apply(pred_instance_one), dim=0)
        target_proj_instance_two = torch.cat(GatherLayer.apply(target_proj_instance_two), dim=0)
        pred_instance_two = torch.cat(GatherLayer.apply(pred_instance_two), dim=0)
        target_proj_instance_one = torch.cat(GatherLayer.apply(target_proj_instance_one), dim=0)
        ...
        if not self.use_pixpro:
            ...
            proj_pixel_one = torch.cat(GatherLayer.apply(proj_pixel_one), dim=0)
            proj_pixel_two = torch.cat(GatherLayer.apply(proj_pixel_two), dim=0)

            target_proj_pixel_two = torch.cat(GatherLayer.apply(target_proj_pixel_two), dim=0)
            target_proj_pixel_one = torch.cat(GatherLayer.apply(target_proj_pixel_one), dim=0)
            ...
            propagated_pixels_one = propagated_pixels_one.to(self.device, copy=False)
            target_proj_pixel_two = target_proj_pixel_two.to(self.device, copy=False)
            propagated_pixels_two = propagated_pixels_two.to(self.device, copy=False)
            target_proj_pixel_one = target_proj_pixel_one.to(self.device, copy=False)
            ...
            loss_pixpro_one_two = - propagated_similarity_one_two.masked_select(positive_mask_one_two[None, ...])
            loss_pixpro_two_one = - propagated_similarity_two_one.masked_select(positive_mask_two_one[None, ...])
            pix_loss = (loss_pixpro_one_two.mean() + loss_pixpro_two_one.mean()) / 2

        # total loss
        loss = pix_loss.to(self.device, copy=False) * self.alpha + instance_loss.to(self.device, copy=False)
    ...
    def training_step(self, batch, batch_idx):
        ...
        log_dict = {"train_loss": loss, "lr": lr}
        if (self.current_epoch + 1) % 5 == 0:
            self.logger.experiment.add_scalar('lr', lr, self.current_epoch)
            torch.save(self.online_encoder.net.state_dict(), 'checkpoint/{}.pth'.format(str(self.current_epoch)))
